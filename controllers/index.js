...
const criarSkate = (req, res) => {
    ...
    const query = 'INSERT INTO skateSetup(modelo , rodinha,rolamentos,trucks,lixa,parafusos,preco) VALUES ($1, $2,$3,$4,$5,$6,$7)'
    const values = [novoSkate.modelo, novoSkate.rodinha, novoSkate.rolamentos, novoSkate.trucks, novoSkate.lixa, novoSkate.parafusos, novoSkate.preco]
...
const atualizarSetup = (req, res) => {
    ...
    const query = 'UPDATE skateSetup SET modelo = $1, rodinha = $2, rolamentos = $3, trucks = $4, lixa = $5, parafusos = $6 preco = $7 WHERE id = $8';
    const values = [skateAtualizado.modelo, skateAtualizado.rodinha, skateAtualizado.rolamentos, skateAtualizado.trucks, skateAtualizado.lixa, skateAtualizado.parafusos, skateAtualizado.preco, skateId];
...
const buscarSkatePorModelo = (req, res) => {
    const modelo = req.params.modelo; // Obtém o modelo do skate a ser buscado

    const query = 'SELECT * FROM skateSetup WHERE modelo ILIKE $1';
    const values = [modelo];
